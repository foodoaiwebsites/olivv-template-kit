 ...
 */
export function defineTemplateManifest(m: TemplateManifest): TemplateManifest {
  const parsed = manifestSchema.parse(m);
  if (parsed.schemaVersion > KIT_SCHEMA_VERSION) {
    throw new Error(
      `@olivv/template-kit: manifest schemaVersion ${parsed.schemaVersion} is newer than this ` +
