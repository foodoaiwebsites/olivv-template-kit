...
export default function BuilderAgent({ allowedOrigin } = {}) {
    useEffect(() => {
        ...
        const reloadVideo = (el) => {
            const v = el.tagName === "SOURCE" ? el.parentElement : el;
            if (v && v.tagName === "VIDEO")
                v.load?.();
        };
        const setImage = (prev, next) => {
            const hit = (cur) => {
                if (!cur)
                    return false;
                if (cur === prev || cur.endsWith(prev) || prev.endsWith(cur))
                    return true;
                try {
            ...
            for (const el of Array.from(document.querySelectorAll("img, video, source"))) {
        ...
        const onMsg = (e) => {
            if (lockedOrigin && e.origin !== lockedOrigin)
                return;
            const d = e.data;
            if (!d || d.__builder !== true)
                return;
            if (d.type === "setText")
                setText(d.prev ?? "", d.next ?? "");
            else if (d.type === "setImage")
                setImage(d.prev ?? "", d.next ?? "");
            else if (d.type === "setVar" && d.name)
                document.body.style.setProperty(d.name, d.value ?? "");
            else if (d.type === "setField" && d.path)
                setField(d.path, d.value ?? "");
        };
