...
export function colorTokenStem(key) {
    if (key === "text")
        return "fg";
    if (key.startsWith("text-"))
        return `fg-${key.slice("text-".length)}`;
    return key;
}
/** Font token key → Tailwind `fontFamily` name (the part after `font-`). */
export function fontTokenName(key) {
 ...
 */
export function tailwindColorsFromTokens(tokens) {
    const out = {};
 ...
 */
export function tailwindFontsFromTokens(tokens, fallback = ["sans-serif"]) {
    const out = {};
    for (const t of tokens) {
        if (t.type !== "font")
            continue;
 ...
 */
export function cssThemeFromTokens(tokens, opts = {}) {
    const lines = [];
    for (const t of tokens) {
        if (t.type === "color") {
